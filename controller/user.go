...
func MyFriends(ctx *gin.Context) {
	uid := ctx.PostForm("uid")
	newUid, _ := strconv.Atoi(uid)
	users := model.GetFriendsById(newUid)
	ctx.JSON(200, gin.H{
		"code":    200,
		"message": "success",
		"data":    users,
...
func UserFollow(ctx *gin.Context) {
	...
	user := model.GetUserInfo(newUid)
	is_follow := model.IsFriend(newUid, newFid)
...
func UpdateNickname(ctx *gin.Context) {
	...
	uid := ctx.PostForm("uid")
	newuid, _ := strconv.Atoi(uid)
	is_update := model.SetUserNickname(newuid, nickname)
	if is_update {
		ctx.JSON(200, gin.H{
			"code":    200,
			"message": "success",
...
func UpdateHeadImg(ctx *gin.Context) {
	...
	is_ok := model.UpdateHeadImg(newUid, headImg)
	if is_ok {
		ctx.JSON(200, gin.H{
			"code":    200,
			"message": "success",
